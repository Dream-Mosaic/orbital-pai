defmodule AppWeb.AppLinkTest do
  ...
  test "an :info outcome becomes an ok status" do
    assert AppLink.connectors(:info) == "henry://connectors?status=ok"
  end

  test "an :error outcome becomes an error status" do
    assert AppLink.connectors(:error) == "henry://connectors?status=error"
  end
  ...
  test "an unrecognized outcome is treated as a failure, not as success" do
    assert AppLink.connectors(:something_new) == "henry://connectors?status=error"
  end
  ...
  test "the link uses the henry scheme and the connectors host" do
