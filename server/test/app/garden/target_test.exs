defmodule App.Garden.TargetTest do
  ...
  test "default (nil) -> household, unconditionally (the garden is shared by default)" do
    assert %{user_id: 1, household: true, assigned: "the household"} = Target.resolve(nil, opts())
    # NOT gated — like Lists (and unlike reminders), household default holds with the gate off.
  ...
  test "household words (including 'the garden') -> household" do
    for word <- ["household", "us", "we", "both", "the house", "the household", "the garden"] do
